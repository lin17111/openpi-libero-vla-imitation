...
def _load_student(model_path: pathlib.Path, device: torch.device) -> StudentBCPolicy:
...
def _preprocess_obs(obs: dict) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    image = np.ascontiguousarray(obs["observation/image"])
    wrist_image = np.ascontiguousarray(obs["observation/wrist_image"])
    state = np.asarray(obs["observation/state"], dtype=np.float32)
    image_t = torch.from_numpy(image).unsqueeze(0)
    wrist_image_t = torch.from_numpy(wrist_image).unsqueeze(0)
    state_t = torch.from_numpy(state).unsqueeze(0)
    return image_t, wrist_image_t, state_t


def _build_student_obs(raw_obs: dict) -> dict:
    ...
    wrist_image = wrist_image.astype(np.uint8, copy=False)
    state = np.asarray(
        np.concatenate(
            (
...
def eval_student_bc(
    ...
    model = _load_student(model_path, device)
        ...
        for episode_idx in range(episodes_this_task):
            ...
            if not done:
                while t < max_steps:
                    ...
                    image_t, wrist_image_t, state_t = _preprocess_obs(student_obs)
                    image_t = image_t.to(device)
                    wrist_image_t = wrist_image_t.to(device)
                    state_t = state_t.to(device)

                    with torch.no_grad():
                        action = model(image_t, wrist_image_t, state_t).squeeze(0).cpu().numpy()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate a trained LIBERO student policy.")
